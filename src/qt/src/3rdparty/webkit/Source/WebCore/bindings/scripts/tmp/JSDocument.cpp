...
static const HashTableValue JSDocumentTableValues[80] =
{
    ...
    { "domain", DontDelete | ReadOnly, (intptr_t)static_cast<PropertySlot::GetValueFunc>(jsDocumentDomain), (intptr_t)0 THUNK_GENERATOR(0) },
...
static const HashTableValue JSDocumentPrototypeTableValues[35] =
{
    ...
    { "createCSSStyleDeclaration", DontDelete | Function, (intptr_t)static_cast<NativeFunction>(jsDocumentPrototypeFunctionCreateCSSStyleDeclaration), (intptr_t)0 THUNK_GENERATOR(0) },
...
JSValue jsDocumentTainted(ExecState* exec, JSValue slotBase, const Identifier&)
{
    ...
    UNUSED_PARAM(exec);
    Document* imp = static_cast<Document*>(castedThis->impl());
    JSValue result = jsBoolean(imp->tainted());
    return result;
}
...
EncodedJSValue JSC_HOST_CALL jsDocumentPrototypeFunctionCreateCSSStyleDeclaration(ExecState* exec)
{
    JSValue thisValue = exec->hostThisValue();
    if (!thisValue.inherits(&JSDocument::s_info))
        return throwVMTypeError(exec);
    ...
    JSC::JSValue result = toJS(exec, castedThis->globalObject(), WTF::getPtr(imp->createCSSStyleDeclaration()));
    return JSValue::encode(result);
}
